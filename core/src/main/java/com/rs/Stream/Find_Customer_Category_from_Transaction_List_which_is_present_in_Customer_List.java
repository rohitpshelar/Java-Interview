...
public class Find_Customer_Category_from_Transaction_List_which_is_present_in_Customer_List {
    ...
    public static void Find_from_SUM_amount(List<Customer> customers) {
        customers.forEach(c -> {
            double totalAmount = c.txns().stream().mapToDouble(Transaction::amount).sum();
    ...
    public static void Find_from_MAX_amount(List<Customer> customers) {
        customers.forEach(c -> {
            double totalAmount = c.txns().stream().mapToDouble(Transaction::amount).max().orElse(0.0);
    ...
    public static void Find_from_SUM_credit_amount(List<Customer> customers) {
        customers.forEach(c -> {
            double totalAmount = c.txns().stream().filter(t -> t.type().equalsIgnoreCase("credit")).mapToDouble(Transaction::amount).sum();
    ...
    public static void main(String[] args) {
        List<Customer> customers = List.of(new Customer(1, "Rohit", List.of(new Transaction(1, "debit", 20000.0), new Transaction(2, "credit", 9000.0), new Transaction(3, "credit", 19000.0))), new Customer(2, "Don", List.of(new Transaction(3, "debit", 60000.0), new Transaction(4, "credit", 20000.0), new Transaction(5, "credit", 40000.0))), new Customer(3, "Samit", List.of(new Transaction(5, "debit", 1000.0), new Transaction(6, "credit", 7000.0), new Transaction(7, "credit", 8000.0))));
